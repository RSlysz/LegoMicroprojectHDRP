    ...
    public class SceneBrickBuilder
    {
        ...
        private static void SetupBrickBuilding(BrickBuildingState newState)
        {
            ...
            switch(newState)
            {
                ...
                {                    
                    ConnectivityUIManager.ShowTools(ConnectivityUIManager.ShouldSHhowTools());
                    EditorApplication.hierarchyWindowItemOnGUI += OnHierarchyGUI;
                    EditorApplication.update += EditorUpdate;
                    ...
                    {
                        brick.UpdateColliding(brick.IsColliding(out _), true, false);
                    }
                }
                break;
        ...
        public static void SetAutoUpdateHierarchy(bool value)
        {
        ...
        private static bool IsBrickValidInHierarchy(Brick brick)
        {
            ...
            // The brick can have a parent unrelated to its connections if it has been moved in the hierarchy
            ...
            if(connected.Intersect(bricksInGroup).Count() != bricksInGroup.Length)
            {
                return false;
            }
            ...
            if (selection != null)
            {
                ...
                {                    
                    ...
                    foreach(var transform in Selection.transforms)
                    {
                        ...
                        {
                            selection.hasChanged = false;
                            RemoveInvalidConnections(b);
                            if(!IsBrickValidInHierarchy(b))
                            {
                                bricksToUpdate.Add(b);
                            }
                        }
                    ...
                    if(bricksToUpdate.Count > 0)
                    {
                        ModelGroupUtility.RecomputeModelGroups(bricksToUpdate, true, ModelGroupUtility.UndoBehavior.withUndo);
                    }
                }
            }
        }
        ...
        private static void NudgeBricks(Camera camera, InteractionKey key)
        {
            ...
            if(autoUpdateHierarchy)
            {
                ModelGroupUtility.RecomputeModelGroups(previouslyColliding.Union(selectedBricks), false);
            }
        }
        ...
        private static void PlaceBrick()
        {
            ...
            if(autoUpdateHierarchy && CurrentSelectionState == SelectionState.moving)
            {
                SyncSceneBricks();
        ...
        private static void HandleDeletedBricks()
        {
            ...
            ModelGroupUtility.RecomputeModelGroups(connected);
        }
        ...
        private static void HandleRotateGizmo(Camera camera)
        {
            ...
            if(selectedBricks.Count > 0 && (CurrentSelectionState == SelectionState.selected || CurrentSelectionState == SelectionState.dragging))
            {
                SyncBounds();
...
#region event handling
        ...
        {
            var fields = StageUtility.GetMainStageHandle().FindComponentsOfType<ConnectionField>();
        ...
        private static void ComputeNewConnection(Camera camera, Ray ray)
        {
            ...
            BrickBuildingUtility.AlignBricks(focusBrick, selectedBricks, selectionBounds, pivot, pickupOffset, ray, worldPlane, BrickBuildingUtility.maxRayDistance, out Vector3 offset, out Vector3 alignedOffset, out Quaternion rotation, out RaycastHit collidingHit);
        ...
        private static void EditorUpdate()
        {
            ...
            if(autoUpdateHierarchy && deleteQueued)
            {                
                HandleDeletedBricks();
            }
        ...
        {
            SyncAndUpdateBrickCollision(false);
        }
        ...
        private static void SyncAndUpdateBrickCollision(bool forceSync = true)
        {
        ...
        private static void ActiveSceneChanged(Scene previous, Scene active)
        {
            if(buildWithBricks)
            {
                ...
                {
                    var bricks = rootObject.GetComponentsInChildren<Brick>();
                    foreach(var brick in bricks)
                    {
                        brick.UpdateColliding(brick.IsColliding(out _), true, false);
                    }
        ...
        {            
            deleteQueued = true;
            var group = destroyedBrick.GetComponentInParent<ModelGroup>();
            if(!group)
            {
                return;
            }
        ...
        {            
            ...
            HashSet<GameObject> models = new HashSet<GameObject>();
            ...
            foreach(var sel in newSelection)
            {
                ...
                if(brick)
                {
                    ...
                    if(model)
                    {
                        ...
                        if (Array.IndexOf(lastSelection, model) < 0 && !newSelection.Contains(model.gameObject))
                        {
            ...
            newSelection.AddRange(models);

            var nonBricks = new List<GameObject>();

            if(changed || newSelection.Count > 0)
            {
                // Find all models and add to selection                
                foreach(var obj in Selection.objects)
                {
                    if(!GetValidGameObject(obj, out GameObject go))
                    {
                    ...
                    var model = go.GetComponentInParent<Model>();
                    if(model != null || (controlDown && model != null && model.gameObject == go))
                    {
                        models.Add(model.gameObject);
                    }
                    else
                ...
                if(controlDown && lastSelection.Length + 1 == Selection.objects.Length)
                {
                    foreach(var obj in Selection.objects)
                    {
                        ...
                        {
                            var model = go.GetComponentInParent<Model>();
                            if(model && Array.IndexOf(lastSelection, model.gameObject) >= 0)
                            {
                                models.Remove(model.gameObject);
                            }
                        }
                    }
                }
            ...
            {
                selection.Add(model.gameObject);
            }
