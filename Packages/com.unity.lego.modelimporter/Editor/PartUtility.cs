...
namespace LEGOModelImporter
{
    ...
    public static class PartUtility
    {
        ...
        // FIXME Add support for just unpacking a given LOD. It is currently complicated by how LOD2 is generated.
        public static bool UnpackCommonPart(string name, bool lightmapped)
        {
            if (File.Exists(Path.Combine(geometryPath, commonPartsDir, lightmapped ? lightmappedDir : "", lod0Dir, name + ".fbx")))
            {
                return true;
            }
            ...
            {
                var fileName = entry.Name;
                var filePathLod0 = Path.Combine(geometryPath, commonPartsDir, lightmapped ? lightmappedDir : "", lod0Dir, fileName);
                var filePathLod1 = Path.Combine(geometryPath, commonPartsDir, lightmapped ? lightmappedDir : "", lod1Dir, fileName);

                var directoryNameLod0 = Path.GetDirectoryName(filePathLod0);
                if (directoryNameLod0.Length > 0)
                {
                    Directory.CreateDirectory(directoryNameLod0);
                }
                var directoryNameLod1 = Path.GetDirectoryName(filePathLod1);
                if (directoryNameLod1.Length > 0)
                {
                    Directory.CreateDirectory(directoryNameLod1);
                }

                var zipStream = entry.Open();
                var fileStream = File.Create(filePathLod0);
                zipStream.CopyTo(fileStream);
                fileStream.Dispose();
                zipStream.Dispose();

                File.Copy(filePathLod0, filePathLod1);
#if UNITY_EDITOR
                AssetDatabase.ImportAsset(filePathLod0);
                AssetDatabase.ImportAsset(filePathLod1);
#endif
                return true;
            }
        ...
        public static bool UnpackConnectivityForPart(string designID)
        {
            if (File.Exists(Path.Combine(connectivityPath, designID + ".prefab")))
            {
                return true;
            }
            ...
            if (entry != null)
            {
                ...
                if (connectivityNode != null)
                {
                    ...
                    foreach (XmlNode connectionField in connectionFields)
                    {
                        ...
                        fieldComponent.connectionGrid = new Connection[featuresPerRow * (fieldComponent.gridSize.y + 1)];
                        ...
                        foreach (var description in connectionDescriptions)
                        {
                            ...
                            // Only create supported connection types.
                            if (ConnectivityUtility.IsConnectionTypeSupported(connectionType))
                            {
                                GameObject connectionGO = new GameObject();
                                connectionGO.layer = LayerMask.NameToLayer(Connection.connectivityFeatureLayerName);

                                var connection = connectionGO.AddComponent<Connection>();

                                connection.quadrants = int.Parse(descs[1], CultureInfo.InvariantCulture);
                                connection.transform.parent = fieldGO.transform;
                                connection.transform.localRotation = Quaternion.identity;

                                connection.transform.localPosition = new Vector3(connectionPositionX, 0.0f, connectionPositionZ);

                                connection.connectionType = connectionType;
                                connection.name = connection.connectionType.ToString();
                                ...
                                var collider = connectionGO.AddComponent<SphereCollider>();
                                collider.isTrigger = true;
                                collider.radius = halfModule * 0.5f;
                                collider.center = Vector3.zero;

                                fieldComponent.connections.Add(connection);

                                var x = features % featuresPerRow;
                                var y = features / featuresPerRow;

                                fieldComponent.connectionGrid[featuresPerRow * y + x] = connection;
                            }

                            connectionPositionX -= halfModule;
                            features++;

                            if (features % featuresPerRow == 0)
                            {
                                connectionPositionZ += halfModule;
                                connectionPositionX = 0.0f;
                            }
                        }

                        // Only add the field if it has any connections.
                        if (fieldComponent.connections.Count > 0)
                        {
        ...
        public static bool UnpackCollidersForPart(string designID)
        {
            ...
            if (entry != null)
            {
                ...
                // FIXME Parse CollisionAdd colliders: Tube and Cylinder.
