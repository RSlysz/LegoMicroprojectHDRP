...
namespace LEGOModelImporter
{
    public class BrickBuildingUtility
    {
        ...
        {
            var hits = physicsScene.Raycast(ray.origin, ray.direction, raycastBuffer, maxDistance, ~LayerMask.GetMask(Connection.connectivityFeatureLayerName), QueryTriggerInteraction.Ignore);
            if(hits > 0)
            {
        ...
        public static (Connection, Connection) FindBestConnection(Vector3 pickupOffset, HashSet<Brick> selectedBricks, Ray ray, Camera camera, Brick[] bricksToCheck, int maxTries = defaultMaxBricksToConsiderWhenFindingConnections)
        {
            ...
            {
                var bestConnectionOnBrick = FindBestConnectionOnBrick(pickupOffset, brick, ray, selectedFields, selectedBricks);
            ...
            if (bestConnectionCandidates.Count > 0)
            {
                ...
                {
                    return Vector3.Distance(ray.origin, c.Item2.transform.position);
                }).ToList();
